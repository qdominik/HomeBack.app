...
W `docs/ops/preview-deployment-contract-2026-08-02.md` występuje 11 razy
realny ref projektu preview Supabase oraz jego URL:

```text
yzewupqxkefyvljnfolk
https://yzewupqxkefyvljnfolk.supabase.co
```
...
- [ ] Zredagować ref `yzewupqxkefyvljnfolk` / URL preview Supabase z docs
      (redakcja zrobiona w working tree, do zacommitowania) oraz
      obrócić/usunąć projekt preview.
- [ ] Ustawić `NEXT_PUBLIC_SITE_URL` produkcyjne i usunąć fallback do nagłówka
      `Origin` w rejestracji.
- [ ] Dodać nagłówki bezpieczeństwa w `next.config.ts`.
- [ ] Usunąć/sparametryzować `192.168.0.205` z `next.config.ts`.
...
- [ ] Zintegrować istniejącą naprawę `4fdf968` (bodySizeLimit 3mb) przed
      zakończeniem PR #16 - nie przepisywać.

...
- [x] Zredagować ref `<SUPABASE_PREVIEW_PROJECT_REF>`/URL preview Supabase z docs
      (redakcja w working tree, do zacommitowania); nadal obrócić/usunąć projekt
      preview.
- [ ] Ustawić `NEXT_PUBLIC_SITE_URL` produkcyjne w konfiguracji wdrożenia
      (właściciel); kod nie korzysta już z fallbacku nagłówka `Origin`.
...
- [ ] Wyrównać `minimum_password_length` do 8 na hosted; decyzja o CAPTCHA
      (Turnstile).
- [x] Ustawić limit `bodySizeLimit` dla Server Actions na `2mb`, zgodny z
      limitem zdjęć; zastępuje niepoprawny limit `3mb` z `4fdf968`.
...
- PR #16 (`codex/item-form-description-layout`): blocker E394 zamknięty przez
  limit Server Actions `2mb`, zgodny z limitem pliku 2 MB.

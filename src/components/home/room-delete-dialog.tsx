...
import {
  deleteRoomWithResolution,
  getRoomDeletionContext,
} from "@/app/(app)/home/actions";
import { Alert } from "@/components/ui/alert";
import { Button, buttonClassName } from "@/components/ui/button";
import {
  canSubmitRoomDelete,
  type RoomDeleteResolution,
...
type LoadedContext = Extract<
  Awaited<ReturnType<typeof getRoomDeletionContext>>,
  { ok: true }
>["context"];
...
export function RoomDeleteDialog({
  ...
  const [resolution, setResolution] =
    useState<RoomDeleteResolution | null>(null);
  ...
  const copy = t.modules.home.roomDelete;
  ...
  async function loadContext() {
    ...
    const result = await getRoomDeletionContext(roomId);
  ...
  async function submitResolution() {
    if (
      !context ||
      !canSubmitRoomDelete(resolution, targetPositionId) ||
      isSubmitting
    ) {
      return;
    }
    ...
    const result = await deleteRoomWithResolution({
      roomId,
      resolution,
      targetPositionId,
      expectedStorageLocationL2Count: context.summary.storageCount,
  ...
  return (
    <>
      ...
      >
        ...
        {t.modules.home.entityLabels.room.singular}
      </button>
      <dialog
          ...
          <div className="mt-5 space-y-5">
            ...
            {isLoading ? (
              ...
              <>
                <SourcePath context={context} />
                <DependencyCounts context={context} />
                {!hasItems && context.summary.storageCount > 0 ? (
...
function DependencyCounts({ context }: { context: LoadedContext }) {
  ...
  const copy = t.modules.home.roomDelete;
  const counts = [
    [copy.furnitureItems, summary.storageCount],

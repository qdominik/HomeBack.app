-- Purpose:
-- Atomically resolve every Item link in one L2 furniture subtree, explicitly
-- delete all nested L3 storage spaces, and then delete the L2 furniture row.
--
-- Data impact:
-- Reuses M4D.3 and M4D.4. No table, column, foreign key, or RLS changes.

create function public.delete_storage_location_l2_with_resolution(
...
returns table (
  status text,
  resolution text,
  deleted_storage_location_l2_id uuid,
...
begin
  ...
  if p_expected_storage_location_l3_count is null
  ...
  if p_resolution = 'move' then
    select l3.storage_location_l2_id
    into v_target_storage_location_l2_id
    ...
    if v_target_storage_location_l2_id = p_storage_location_l2_id then
  ...
  from public.get_storage_location_l2_dependency_summary(
    p_storage_location_l2_id
  ) as summary;

  if v_storage_location_l3_count <> p_expected_storage_location_l3_count
  ...
  begin
    ...
    if p_resolution in ('detach', 'move') then
      ...
      from public.detach_items_from_storage_location_l2(
        p_storage_location_l2_id
      ) as detached;
    end if;
    ...
    if v_deleted_storage_location_l3_count <> v_storage_location_l3_count then
      raise exception 'storage subtree delete count mismatch';
    end if;
    ...
    if v_deleted_storage_location_l2_count <> 1 then
      raise exception 'furniture delete did not affect exactly one row';
    end if;
...
revoke all on function public.delete_storage_location_l2_with_resolution(
  uuid,
  text,
  uuid,
  bigint,
...
grant execute on function public.delete_storage_location_l2_with_resolution(
  uuid,
  text,
  uuid,
  bigint,
...
comment on function public.delete_storage_location_l2_with_resolution(
  uuid,
  text,
  uuid,
  bigint,
  ...
  'Atomically deletes one L2 furniture subtree after deleting, detaching, or moving every Item link for the active household admin.';
